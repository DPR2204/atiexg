  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative bg-white w-full max-w-5xl max-h-[92vh] rounded-[2.5rem] sm:rounded-[3rem] overflow-hidden shadow-2xl flex flex-col md:flex-row">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 sm:top-6 sm:right-6 z-10 bg-white/20 backdrop-blur text-white p-2 rounded-full hover:bg-white/40 md:text-gray-900 md:bg-gray-100"
          aria-label={`Cerrar detalle de ${tour.name}`}
        >
        ...
        <div className="flex-1 p-6 sm:p-8 md:p-12 overflow-y-auto scrollbar-hide bg-white space-y-8 sm:space-y-10">
          ...
          <div className="pt-2">
            <button
              ...
              aria-label={selection ? 'Actualizar selección del plan' : 'Agregar esta experiencia al plan'}
            >
...
export const Header = ({ onSearchClick }: { onSearchClick: () => void }) => {
    ...
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-xl border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 h-16 sm:h-20 flex items-center justify-between gap-3 sm:gap-6">
        ...
        <nav className="hidden lg:flex items-center gap-6 text-[10px] font-mono uppercase tracking-[0.3em] text-gray-400">
          <Link to="/#inicio" className="hover:text-gray-900 transition-colors">Inicio</Link>
          <Link to="/catalogo" className="hover:text-gray-900 transition-colors">Catálogo</Link>
          <Link to="/conocenos" className="hover:text-gray-900 transition-colors">Conócenos</Link>
          <Link to="/contacto" className="hover:text-gray-900 transition-colors">Contacto</Link>
        </nav>
        <button
          onClick={handleSearchClick}
          className="hidden md:flex items-center flex-1 max-w-md gap-2 sm:gap-3 px-3.5 py-2 sm:px-5 sm:py-2.5 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all text-left group active:scale-[0.98] bg-white/50 min-w-0"
          aria-label="Abrir búsqueda de experiencias"
        >
        ...
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          ...
          <Link to="/contacto" className="hidden sm:block text-sm font-bold text-gray-600 hover:text-red-500">Contacto</Link>
          <Link to="/catalogo" className="hidden sm:inline-flex bg-black text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-sm font-black hover:bg-red-500 active:scale-95 transition-all">CATÁLOGO</Link>
          <button
      ...
      {isMobileMenuOpen && (
        <div className="sm:hidden border-t border-gray-100 bg-white/95 backdrop-blur-xl px-4 pb-4">
          <div className="pt-4 grid gap-3 text-[10px] font-mono uppercase tracking-[0.3em] text-gray-500">
            <Link to="/#inicio" onClick={closeMenu} className="hover:text-gray-900">Inicio</Link>
            <Link to="/catalogo" onClick={closeMenu} className="hover:text-gray-900">Catálogo</Link>
            <Link to="/conocenos" onClick={closeMenu} className="hover:text-gray-900">Conócenos</Link>
            <Link to="/contacto" onClick={closeMenu} className="hover:text-gray-900">Contacto</Link>
          </div>
          <div className="mt-4 flex gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex-1 text-center bg-green-600 text-white py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest"
              aria-label="Abrir WhatsApp de Atitlán Experiences"
            >
              WhatsApp
            </a>
            <Link to="/catalogo" onClick={closeMenu} className="flex-1 text-center bg-black text-white py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest">Catálogo</Link>
          </div>
        </div>
      )}
    </header>
    ...
    <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div className="w-full max-w-lg bg-white p-6 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl relative">
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          ...
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition-colors" aria-label="Cerrar filtros avanzados">
            <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
...
const HomePage = ({
  ...
  const toggleGeneralAddon = (label: string) => {
    setSelectedGeneralAddons(prev => prev.includes(label) ? prev.filter(item => item !== label) : [...prev, label]);
  };
          ...
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] items-center">
            <div>
              ...
              <div className="mt-6 sm:mt-8 flex flex-wrap gap-3">
                <Link to="/catalogo" className="bg-black text-white px-5 sm:px-7 py-3 sm:py-4 rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest hover:bg-red-500">Ver catálogo</Link>
                <Link to="/contacto" className="bg-white text-gray-900 px-5 sm:px-7 py-3 sm:py-4 rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest border border-gray-200 hover:border-gray-300">Contactar</Link>
              </div>
            </div>
            ...
            {filteredTours.map(tour => (
              <div key={tour.id} className="relative group">
                <button
                  ...
                  aria-label={selectedConfigs.some(c => c.tourId === tour.id) ? `Quitar ${tour.name} del plan` : `Agregar ${tour.name} al plan`}
                >
                  ...
                  <article className="bg-white rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-gray-200 shadow-sm group-hover:shadow-md transition-all duration-500 group-hover:-translate-y-2">
                    ...
                    <div className="p-5 sm:p-7 flex justify-between items-center bg-white border-t border-gray-50">
                      ...
                      <Link
                        ...
                        className="bg-black text-white px-5 sm:px-7 py-3 sm:py-4 rounded-xl sm:rounded-[1.25rem] font-black text-[10px] sm:text-xs uppercase tracking-widest hover:bg-red-500 transition-all shadow-sm"
                        aria-label={`Ver detalle de ${tour.name}`}
                      >
                        DETALLE
                      </Link>
                    </div>
                          ...
                          return (
                            <li key={item}>
                              <button
                                ...
                                className={`w-full flex items-start gap-2 rounded-xl px-3 py-2 text-left text-[12px] sm:text-sm font-bold border transition-colors ${isSelected ? 'bg-emerald-600/10 border-emerald-200 text-emerald-700' : 'bg-white border-gray-100 text-gray-600 hover:border-emerald-100 hover:text-emerald-600'}`}
                              >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="bg-green-600 text-white px-5 sm:px-7 py-3 sm:py-4 rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest hover:bg-green-500"
