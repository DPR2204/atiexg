...
const values = TOURS.map(t => {
    const sanitize = (str) => str ? `'${str.replace(/'/g, "''")}'` : 'NULL';
    const sanitizeJson = (obj) => obj ? `'${JSON.stringify(obj).replace(/'/g, "''")}'` : 'NULL';
    const sanitizeArray = (arr) => arr ? `ARRAY[${arr.map(s => `'${s.replace(/'/g, "''")}'`).join(',')}]` : 'NULL';
    ...
    ${sanitize(t.duration)},
    ${sanitize(t.image)},
    ${sanitizeArray(t.gallery)},
    ${t.isBestSeller ? 'true' : 'false'},
    ${t.isNew ? 'true' : 'false'},
    ${t.rating},
    ${t.reviews},
    ${sanitizeArray(t.features)},
    ${sanitize(t.includes)},
    ${sanitizeJson(t.itinerary)},
    ${sanitizeJson(t.prices)},
    ${sanitizeJson(t.addons)},
    ${sanitizeArray(t.meals)},
    ${sanitize(t.format)}
  )`;
}).join(',\n');
