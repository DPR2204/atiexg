...
const DestinoPage = () => {
      ...
      <div className="min-h-screen bg-white">
        ...
        <main className="max-w-5xl mx-auto px-4 py-16 animate-fade-in-up">
          <div className="text-center">
            ...
            <h2 className="text-2xl sm:text-4xl font-black text-gray-900 mb-4">{t('destination.notFoundTitle')}</h2>
            <p className="text-gray-500 text-sm sm:text-base font-medium mb-8 max-w-md mx-auto">
              {t('destination.notFoundDesc')}
            </p>
            <Link
              to="/catalogo"
  ...
  const localizedHighlights = LArray(destination, 'highlights', language);
  const localizedActivities = LArray(destination, 'activities', language);
  ...
  const waMessage = t('destination.waMessage').replace('{name}', destination.name);
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-12 lg:p-16">
            <div className="max-w-7xl mx-auto animate-fade-in-up">
              ...
              <p className="text-lg sm:text-xl text-white/80 max-w-2xl">
                {L(destination, 'subtitle', language)}
              </p>

              {/* Quick stats */}
              <div className="flex flex-wrap gap-4 mt-6">
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-2">
                  <p className="text-[10px] font-mono uppercase tracking-wider text-white/60">{t('destination.travelTime')}</p>
                  <p className="text-white font-bold text-sm">{L(destination, 'travelTime', language)}</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-2">
                  <p className="text-[10px] font-mono uppercase tracking-wider text-white/60">{t('destination.bestTime')}</p>
                  <p className="text-white font-bold text-sm">{L(destination, 'bestTime', language)}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="animate-fade-in-up">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs font-bold uppercase tracking-[0.3em] text-red-500">
                    {t('destination.aboutTag')}
                  </span>
                </div>
                <h2 className="text-3xl sm:text-4xl font-black text-gray-900 leading-tight mb-6">
                  {t('destination.aboutTitle')} {destination.name}
                </h2>
                <p className="text-gray-600 leading-relaxed text-base sm:text-lg">
                  {L(destination, 'description', language)}
                </p>
              </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-10 animate-fade-in-up">
              <span className="inline-block text-xs font-bold uppercase tracking-[0.3em] text-red-500 mb-4">
                {t('destination.galleryTag')}
              </span>
              <h2 className="text-3xl sm:text-4xl font-black text-gray-900 leading-tight">
                {t('destination.galleryTitle')} {destination.name}
              </h2>
            </div>
              ...
              {destination.gallery.map((image, index) => (
                ...
                >
                  <img
                    ...
                    alt={`${destination.name} - ${language === 'en' ? 'image' : 'imagen'} ${index + 1}`}
        ...
        <section className="py-16 lg:py-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-12 animate-fade-in-up">
              <span className="inline-block text-xs font-bold uppercase tracking-[0.3em] text-red-500 mb-4">
                {t('destination.activitiesTag')}
              </span>
              <h2 className="text-3xl sm:text-4xl font-black text-gray-900 leading-tight">
                {t('destination.activitiesTitle')} {destination.name}
              </h2>
            </div>
            ...
            <div className="relative glass-card rounded-[2.5rem] overflow-hidden">
              ...
              <div className="relative px-8 py-14 sm:px-14 sm:py-20 text-center">
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black text-white leading-tight mb-4">
                  {t('destination.ctaVisit')} {destination.name} {t('destination.ctaWith')}
                </h2>
                <p className="text-white/70 max-w-xl mx-auto mb-8">
                  {t('destination.ctaDesc')} {destination.name} {t('destination.ctaAndLake')}
                </p>
                <div className="flex flex-wrap justify-center gap-4">
                  <a
                    href={`https://wa.me/50222681264?text=${encodeURIComponent(waMessage)}`}
                    target="_blank"
                    rel="noreferrer"
                ...
                >
                  <div className="relative aspect-[3/2] overflow-hidden">
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                      ...
                      <p className="text-white/80 text-sm">{L(dest, 'subtitle', language)}</p>
                    </div>
                  </div>
                  <div className="p-4">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-500">{L(dest, 'travelTime', language)}</span>
